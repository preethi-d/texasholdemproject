...
\end{displaymath}
Where $t$ is the number of turns between the move and the terminal node, $R_t$ is the reward for round $t$, $\lambda$ is the discount factor, $s$ is the state and $a$ is the action.
...
The value of $\epsilon$ will slowly decrease as the agent acquires more training. A relatively large initial value ensures that all possible paths will be explored before the agent settles into a sub-optimal pattern. The $i$-th $State$ and $Action$ for the $\epsilon$-Greedy Algorithm is
\begin{displaymath}
...
\item [\emph{S}]refers to the current Street that the game is in, which is the start of each new round. $S$ three groups that are evaluated are Flop, Turn and River.
\item [\emph{P}]refers to the Pot Size, which is the total amount in the player's bet. $P$ is grouped by the number of big blinds.
\item [\emph{\#OR}]refers to the number of Opponent Raises per street. $OR$ is grouped into nine different groups, i.e. from 0 to 8.
\item [\emph{\#SR}]refers to the number of Self Raises per street. $SR$ is grouped into nine different groups, i.e. from 0 to 8.
